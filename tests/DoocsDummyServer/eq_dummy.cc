...
eq_dummy::eq_dummy()
    : EqFct("NAME = location"),
      ...
      prop_someFloat("SOME_FLOAT", this), prop_someDouble("SOME_DOUBLE", this),
      ...
      prop_someIIII("SOME_IIII", this),
      prop_unsupportedDataType("UNSUPPORTED_DATA_TYPE", this),
      prop_someZMQInt("SOME_ZMQINT", this), counter(0) {}
...
void eq_dummy::init() {
  ...
  for (int i = 0; i < 42; i++)
    prop_someIntArray.set_value(3 * i + 120, i);
  for (int i = 0; i < 5; i++)
    prop_someShortArray.set_value(10 + i, i);
  for (int i = 0; i < 5; i++)
    prop_someLongArray.set_value(10 + i, i);
  for (int i = 0; i < 5; i++)
    prop_someFloatArray.set_value((float)i / 1000., i);
  for (int i = 0; i < 5; i++)
    prop_someDoubleArray.set_value((float)i / 333., i);
  for (int i = 0; i < 100; i++)
    prop_someSpectrum.fill_spectrum(i, (float)i / 42.);
...
void eq_dummy::post_init() {
  ...
  if (res != 0) {
    std::cout << "Could not enable ZeroMQ messaging for prop_someZMQInt. Code: "
              << res << std::endl;
    exit(1);
  }
}
