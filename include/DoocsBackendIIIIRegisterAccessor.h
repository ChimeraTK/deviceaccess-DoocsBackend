...
class DoocsBackendIIIIRegisterAccessor
    : public DoocsBackendIntRegisterAccessor<UserType> {
protected:
  using DoocsBackendIntRegisterAccessor<
      UserType>::DoocsBackendIntRegisterAccessor;
...
void DoocsBackendIIIIRegisterAccessor<UserType>::doPreWrite() {
  ...
  if (!DoocsBackendRegisterAccessor<UserType>::isArray) {
    int32_t raw =
        DoocsBackendIntRegisterAccessor<UserType>::fixedPointConverter.toRaw(
    ...
    if (DoocsBackendRegisterAccessor<
      ...
      DoocsBackendRegisterAccessor<UserType>::src.set(
          DoocsBackendRegisterAccessor<UserType>::dst.get_int(0),
    ...
    DoocsBackendRegisterAccessor<UserType>::src.set(d->i1_data, d->i2_data,
                                                    d->i3_data, d->i4_data);
  }
}
