...
namespace mtca4u {
    ...
    void DoocsBackendIIIIRegisterAccessor<UserType>::doPreWrite() {
      ...
      if(!DoocsBackendRegisterAccessor<UserType>::isArray) {
        int32_t raw = DoocsBackendIntRegisterAccessor<UserType>::fixedPointConverter.toRaw(NDRegisterAccessor<UserType>::buffer_2D[0][0]);
      ...
      for(size_t i=0; i<DoocsBackendRegisterAccessor<UserType>::nElements; i++) {
        int32_t raw = DoocsBackendIntRegisterAccessor<UserType>::fixedPointConverter.toRaw(NDRegisterAccessor<UserType>::buffer_2D[0][i]);
