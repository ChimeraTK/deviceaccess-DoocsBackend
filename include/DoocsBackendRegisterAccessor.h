...
protected:
  DoocsBackendRegisterAccessor(const std::string &path, size_t numberOfWords,
  ...
  bool mayReplaceOther(
    ...
    auto rhsCasted = boost::dynamic_pointer_cast<
        const DoocsBackendRegisterAccessor<UserType>>(other);
    if (!rhsCasted)
      return false;
    if (_path != rhsCasted->_path)
      return false;
    return true;
  }
...
DoocsBackendRegisterAccessor<UserType>::DoocsBackendRegisterAccessor(
    ...
    : NDRegisterAccessor<UserType>(path), _path(path),
      elementOffset(wordOffsetInRegister), useZMQ(false) {
  try {
    ...
    if (flags.has(AccessMode::wait_for_new_data)) {
      ...
      activeFuture = TransferFuture(
          notifications.then<void>([](EqData &) {}, std::launch::deferred),
          this);
      futureCreated = true;
      ...
      if (err) {
        throw ChimeraTK::runtime_error(
            std::string("Cannot subscribe to DOOCS property via ZeroMQ: ") +
            dst.get_string());
      }
      // run dmsg_start() once
  ...
  if (!useZMQ) {
    ...
    while (true) {
      ...
      if (dst.error() != 0) {
        ...
        if (rc) {
          throw ChimeraTK::runtime_error(
              std::string("Cannot read from DOOCS property: ") +
              dst.get_string());
        }
        // otherwise try again
        continue;
...
void DoocsBackendRegisterAccessor<UserType>::doReadTransfer() {
  ...
  if (!useZMQ) {
    ...
    if (rc) {
      throw ChimeraTK::runtime_error(
          std::string("Cannot read from DOOCS property: ") + dst.get_string());
    }
  } else {
    // loop to allow retries in case of timeout errors
    while (true) {
...
void DoocsBackendRegisterAccessor<UserType>::write_internal() {
  ...
  if (rc || dst.error() != 0) {
    throw ChimeraTK::runtime_error(
        std::string("Cannot write to DOOCS property: ") + dst.get_string());
  }
}
...
template <typename UserType>
void DoocsBackendRegisterAccessor<UserType>::zmq_callback(void *self_,
                                                          EqData *data,
                                                          dmsg_info_t *) {

  // obtain pointer to accessor object
  DoocsBackendRegisterAccessor<UserType> *self =
      static_cast<DoocsBackendRegisterAccessor<UserType> *>(self_);
...
TransferFuture DoocsBackendRegisterAccessor<UserType>::doReadTransferAsync() {
  ...
  if (!futureCreated) {
    ...
    activeFuture =
        TransferFuture(notifications.then<void>([](EqData &) {}), this);
    futureCreated = true;
  }
