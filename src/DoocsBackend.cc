...
public:
  ...
  bool isWriteable() const override {
    return true;
  } /// @todo fixme: return true for read-only properties

  ChimeraTK::AccessModeFlags getSupportedAccessModes() const override {
    return {};
  } /// @todo can we determine if zeromq can be used?

  const ChimeraTK::RegisterInfo::DataDescriptor &
  getDataDescriptor() const override {
...
DoocsBackend::BackendRegisterer::BackendRegisterer() {
  std::cout << "DoocsBackend::BackendRegisterer: registering backend type doocs"
            << std::endl;
  ChimeraTK::BackendFactory::getInstance().registerBackendType(
      "doocs", &DoocsBackend::createInstance,
      {"facility", "device", "location"});
}
...
DoocsBackend::createInstance(std::string address,
...
void DoocsBackend::fillCatalogue(std::string fixedComponents,
                                 long level) const {
  ...
  for (int i = 0; i < dst.array_length(); ++i) {
    ...
    if (level < 2) {
      ...
      boost::shared_ptr<DoocsBackendRegisterInfo> info(
          new DoocsBackendRegisterInfo());
      ...
      if (dst.type() == DATA_TEXT ||
          dst.type() == DATA_STRING || // in case of strings, DOOCS reports the
        ...
        info->dataDescriptor = ChimeraTK::RegisterInfo::DataDescriptor(
            ChimeraTK::RegisterInfo::FundamentalType::numeric, false, true, 320,
            300);
      }
...
void DoocsBackend::fillCatalogue() const {
  if (!catalogueFilled && _opened) {
    ...
    if (!boost::starts_with(_serverAddress, "doocs://")) {
      ...
      nSlashes =
          std::count(_serverAddress.begin(), _serverAddress.end(), '/') - 3;
    }
...
DoocsBackend::getRegisterAccessor_impl(const RegisterPath &registerPathName,
  ...
  int rc = eq.get(&ea, &src, &dst);
  if (rc) {
    throw ChimeraTK::runtime_error(std::string("Cannot open DOOCS property: ") +
                                   dst.get_string());
  }
  ...
  if (dst.type() == DATA_INT || dst.type() == DATA_A_INT ||
    ...
    p = new DoocsBackendIntRegisterAccessor<UserType>(
        path, numberOfWords, wordOffsetInRegister, flags);
    ...
    p = new DoocsBackendLongRegisterAccessor<UserType>(
        path, numberOfWords, wordOffsetInRegister, flags);
             ...
             dst.type() == DATA_SPECTRUM) {
    p = new DoocsBackendFloatRegisterAccessor<UserType>(
        path, numberOfWords, wordOffsetInRegister, flags);
  } else if (dst.type() == DATA_TEXT || dst.type() == DATA_STRING ||
             dst.type() == DATA_STRING16) {
